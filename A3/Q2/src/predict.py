...
def _live_inference_C(model_dict, dataset, pos, neg, split):
    ...
    from train_C import GATv2Encoder, LinkPredictor, compute_struct_features, NUM_STRUCT_FEATS
    ...
    # Compute structural features for these edges
    struct_pos = compute_struct_features(
        dataset.edge_index, dataset.num_nodes, pos
    ).to(device)
    struct_neg = compute_struct_features(
        dataset.edge_index, dataset.num_nodes, neg.reshape(-1, 2)
    ).to(device)

    ensemble_pos = []
    ensemble_neg = []

    for enc_state, pred_state, w in zip(encoder_states, predictor_states, weights):
        ...
        ps = predictor(
            emb[pos[:, 0].long()], emb[pos[:, 1].long()], struct_pos
        ).cpu()
        ns = predictor(
            emb[neg.reshape(-1, 2)[:, 0].long().to(device)],
            emb[neg.reshape(-1, 2)[:, 1].long().to(device)],
            struct_neg
        ).reshape(P, K).cpu()

        ensemble_pos.append(ps * w)
        ensemble_neg.append(ns * w)

    pos_scores = torch.stack(ensemble_pos).sum(dim=0)
    neg_scores = torch.stack(ensemble_neg).sum(dim=0)
